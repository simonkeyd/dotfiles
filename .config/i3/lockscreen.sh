...
echo "done1"
if [[ -f $HOME/Images/lockscreen-logo.png ]]
then
    # placement x/y
    PX=0
    PY=0
    # lockscreen image info
    R=$(file ~/Images/lockscreen-logo.png | grep -o '[0-9]* x [0-9]*')
    RX=$(echo $R | cut -d' ' -f 1)
    RY=$(echo $R | cut -d' ' -f 3)
    ...
    do
        ...
        convert /tmp/screen.png $HOME/Images/lockscreen-logo.png -geometry +$PX+$PY -composite -matte  /tmp/screen.png
        echo "done"
    done
fi
